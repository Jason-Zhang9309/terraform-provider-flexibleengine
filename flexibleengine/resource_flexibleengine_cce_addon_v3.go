...
func resourceCCEAddonV3Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error retrieving flexibleengine CCEAddon: %s", err)
	}
...
func waitForCCEAddonDelete(cceClient *golangsdk.ServiceClient, id, clusterID string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete flexibleengine CCE Addon %s.\n", id)
		...
		log.Printf("[DEBUG] flexibleengine CCE Addon %s still available.\n", id)
